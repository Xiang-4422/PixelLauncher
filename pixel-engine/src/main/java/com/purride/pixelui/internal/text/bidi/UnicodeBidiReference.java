 ...
 * is implemented in a new UnicodeBidiPairedBracketReference class.
 * <p>
 ...
 *
 * <p>
 * This implementation is not optimized for performance. Its control flow is the
 * Unicode-published 2016 reference core last revised for Unicode 8.0. Pixel Engine supplies fixed
 * Unicode 17.0.0 Bidi_Class, paired-bracket, and mirroring data around that core, and verifies the
 * result against the complete Unicode 17 BidiTest and BidiCharacterTest corpora for UAX #9
 * revision 51. Production resolution therefore does not inherit the host JVM or Android ICU
 * Unicode version.
 * <p>
 * <b>Input:</b><br>
...
final class UnicodeBidiReference {
    /** Immutable Bidi_Class input aligned with original paragraph code points. */
    private final byte[] initialTypes;
    /** Sentinel requesting P2/P3 implicit paragraph-level detection. */
    public final static byte implicitEmbeddingLevel = 2;
    /** Explicit 0/1 base level or [implicitEmbeddingLevel] before P2/P3 resolution. */
    private byte paragraphEmbeddingLevel = implicitEmbeddingLevel;

    /** Number of original paragraph code points represented by every aligned input array. */
    private int textLength;
    /** Resolved directional types for the paragraph before line-only rules. */
    private byte[] resultTypes;
    /** Resolved embedding levels for the paragraph before line-only rules. */
    private byte[] resultLevels;

    /** Returns a defensive copy of resolved paragraph types for conformance diagnostics. */
    public byte [] getResultTypes() { return resultTypes.clone(); }
     ...
     */
    /** Paired-bracket type aligned with every original paragraph code point. */
    private final byte[] pairTypes;
    /** Canonical-equivalence-aware paired-bracket identity aligned with the paragraph. */
    private final int[] pairValues;

    /** Latest paired-bracket resolver retained only for reference diagnostics. */
    public UnicodeBidiPairedBracketReference pba;
     ...
     *               as defined in UnicodeBidiPairedBracketReference (note, both opening and closing
     *               bracket get the same value if they are part of the same canonical "set"
     *               or pair)
     */
    public UnicodeBidiReference(byte[] types, byte [] pairTypes, int [] pairValues) {
        validateTypes(types);
     ...
     */
    public UnicodeBidiReference(byte[] types, byte [] pairTypes, int [] pairValues, byte paragraphEmbeddingLevel) {
        validateTypes(types);
        validatePbTypes(pairTypes);
        validatePbValues(pairValues, pairTypes);
    ...
    /** Maximum explicit embedding depth specified by UAX #9. */
    public static final int MAX_DEPTH = 125;

    /** Stores explicit embedding levels, overrides, and isolate states for X1 through X8. */
    private class directionalStatusStack {
        /** Number of initialized stack entries. */
        private int stackCounter = 0;
        /** Embedding level held by every stack entry. */
        private final byte[] embeddingLevelStack = new byte[MAX_DEPTH + 1];
        /** Directional override type held by every stack entry. */
        private final byte[] overrideStatusStack = new byte[MAX_DEPTH + 1];
        /** Whether every stack entry was introduced by an isolate initiator. */
        private final boolean[] isolateStatusStack = new boolean[MAX_DEPTH + 1];

        /** Removes every logical stack entry without clearing unused backing bytes. */
        public void empty() {
            stackCounter = 0;
        }

        /** Pushes one validated explicit directional status. */
        ...
        /** Removes the current top entry; callers guarantee the stack is non-empty. */
        public void pop() {
            --stackCounter;
        }

        /** Returns the current number of logical entries. */
        public int depth() {
            return stackCounter;
        }

        /** Returns the embedding level at the top entry. */
        public byte lastEmbeddingLevel() {
            return embeddingLevelStack[stackCounter - 1];
        }

        /** Returns the directional override type at the top entry. */
        public byte lastDirectionalOverrideStatus() {
            return overrideStatusStack[stackCounter - 1];
        }

        /** Returns whether the top entry represents an isolate. */
    ...
    /** One BD13 isolating run sequence resolved independently through W1–I2. */
    private class IsolatingRunSequence {
        /** Map from sequence positions to original paragraph indexes. */
        private final int[] indexes;
        /** Mutable resolved type aligned with each sequence position. */
        private final byte[] types;
        /** Resolved level aligned with each sequence position after I1/I2. */
        private byte[] resolvedLevels;
        /** Number of code points in this isolating run sequence. */
        private final int length;
        /** Embedding level shared by the isolating run sequence. */
        private final byte level;
        /** Strong start-of-sequence and end-of-sequence types from X10. */
        ...
        {
            pba = new UnicodeBidiPairedBracketReference();
    ...
    private static void validatePbTypes(byte[] pairTypes) {
        ...
        for (int i = 0; i < pairTypes.length; ++i) {
            if (pairTypes[i] < UnicodeBidiPairedBracketReference.n || pairTypes[i] > UnicodeBidiPairedBracketReference.c) {
     ...
     *            the externally supplied paragraph embedding level.
     */
    public static final UnicodeBidiReference analyzeInput(byte[] types, byte [] pairTypes, int [] pairValues, byte paragraphEmbeddingLevel) {
        UnicodeBidiReference bidi = new UnicodeBidiReference(types, pairTypes, pairValues, paragraphEmbeddingLevel);
        return bidi;
    }
