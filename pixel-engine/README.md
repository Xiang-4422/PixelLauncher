...
- **导航与动画系统**：`PixelNavigator` / `PixelRoute` 支持 slide transition、guard、lifecycle、snapshot / Android Bundle restore，`PixelRouteScrollRestoration` 自动恢复 route 内滚动位置；动画基础包含 `PixelAnimationController`、`TweenAnimationBuilder`、`AnimatedContainer`、`AnimatedSwitcher`、`AnimatedBuilder`
