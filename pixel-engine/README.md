...
- **宿主环境适配**：`MediaQuery` 暴露逻辑尺寸、系统 `viewPadding/padding` 与 IME `viewInsets`，`SafeArea` 自动避让系统栏 / cutout 区域
...
- **表单与焦点**：`Form` / `FormField` 支持字段、跨字段与异步校验，`FormSubmitState` 覆盖提交状态；`FocusScope`、键盘方向遍历和 TextField IME `NEXT` 可直接联动
