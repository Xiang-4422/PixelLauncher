...
`PixelBufferPool`（在 `com.purride.pixelcore.graphics`）按尺寸维护两张独立 HashMap：
- `monoPools`：`MonoPixelBuffer`（`ByteArray`），Mono 模式路径
- `colorPools`：`ColorPixelBuffer`（`IntArray`），Color 模式路径

`acquireMono(w, h)` / `acquireColor(w, h)` 分别借用；`release(buf)` 按类型归还。两表隔离，避免跨类型 key 碰撞。池预热后每帧分配接近 0。
