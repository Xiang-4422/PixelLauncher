...
一个 Flutter 风格的像素艺术 UI SDK。它**不依赖** Android 的 View 树管理 UI，但**通过**一个继承自 `android.view.View` 的 `PixelHostView` 作为宿主——把所有内容渲染到 `PixelBuffer`（ARGB `IntArray`），然后在 `onDraw(Canvas)` 里把整块 buffer 画上去。widget 是不可变的 Kotlin 数据对象；引擎把它们翻译成 retained element 树驱动 layout / paint / hit-test。
...
┌─────────────────────────────────────────────────────────┐
│  Widget 层  （不可变配置对象）                          │
│  Row, Column, Text, Padding, Container, OutlinedButton… │
...
`RenderBox` 子类实现 `layout(constraints)` 和 `paint(context, offsetX, offsetY)`。管线 `PipelineOwner.render()` 深度优先遍历：先 layout，后 paint。结果落到 `PixelBuffer`（每像素是 ARGB 32-bit `Int`）。
...
`PixelBufferPool`（在 `com.purride.pixelcore`）按 `(width, height)` 分桶复用 `PixelBuffer`。当前 `PixelBuffer` 是单一 ARGB `IntArray` 实现，不再区分 Mono / Color buffer。

`acquire(w, h)` 命中时会清零复用，未命中时新建；`release(buf)` 按尺寸归还。池预热后每帧分配接近 0。
...
| 命名空间 | 内容 | 备注 |
|---|---|---|
| `com.purride.pixelui` (扁平根) | 所有 widget（`Text`、`Row`、`Column`、`OutlinedButton`……）、样式类型（`EdgeInsets`、`PixelTextStyle`、`PixelButtonStyle`、`PixelTextFieldStyle`……）、宿主类型（`PixelHostView`、`PixelHostSetupConfig`、`PixelHostBridge`……）、框架基础类（`Widget`、`BuildContext`、`Directionality`、`MediaQuery`、`DefaultTextRasterizer`……）| 大部分公开 API 在这里 |
...
│   ├── pixelcore/           ← 纯绘图层（PixelBuffer / PixelColor / PixelBitmap / 字体引擎），不依赖 Android UI
...
- 想用 SDK 构建应用 → 回到 [README](../../README.md) → [快速入门](../使用说明/快速入门.md)
- 想扩展 SDK 能力 → [扩展开发](../使用说明/扩展开发.md)
