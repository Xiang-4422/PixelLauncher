...
默认恢复策略是 `PixelListRestorationPolicy.AbsoluteOffset`，保持旧行为并把 offset 夹紧到新范围。`RelativeProgress` 使用 `PixelListSavedState.maxScrollOffsetPx` 把原滚动百分比映射到新范围；当保存时没有有效滚动范围时会回退到绝对 offset。两种策略都会清除 drag/fling 中间状态。
