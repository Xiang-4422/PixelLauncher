...
默认的 Mono 模式只有三个 tone（OFF/ON/ACCENT）。如需任意 ARGB 颜色，启用 **Color 模式**：
...
val setup = createPixelHostSetup(
    context = this,
    config = PixelHostSetupConfig(
        colorMode = PixelColorMode.Color,               // ← 关键
        themeData = ThemeData(tokens = PixelThemeTokens.Dark),
        content = {
            Container(
                width = 32,
                height = 16,
                fillColor = PixelColor.fromRgb(30, 80, 200),   // 蓝底
                borderColor = PixelColor.fromRgb(255, 200, 0), // 黄框
                child = Text("COLOR!", color = PixelColor.fromRgb(255, 255, 255)),
            )
        },
    ),
)
...
> `colorMode` 在构造时固定，运行期不可变。
>
> 详细说明见 [主题系统 → 彩色模式](主题系统.md#彩色模式)。
