...
除了三色阶 Mono 模式，pixel-engine 还支持任意 ARGB 的 **Color 模式**。
...
createPixelHostSetup(
    ...
    config = PixelHostSetupConfig(
        ...
        themeData = ThemeData(tokens = PixelThemeTokens.Dark),
        content = { MyColorWidget() },
    ),
)
```

> **注意**：`colorMode` 在构造时固定，不支持运行时动态切换。

### 双字段 tokens

每个 `PixelThemeTokens` 字段都有对应的 `xxxColor: PixelColor?`：

| Mono 字段 | Color 字段 | 含义 |
|---|---|---|
| `textTone` | `textColor` | 默认文本色 |
| `accentTextTone` | `accentTextColor` | 强调文本色 |
| `containerFillTone` | `containerFillColor` | 容器填充色 |
| `containerBorderTone` | `containerBorderColor` | 容器边框色 |
| … | … | … |

SDK 预设（`Dark`、`Light`、`Ocean`、`Amber`）已在 Color 模式下填入默认颜色值。

### resolve 行为（Color 模式）

1. widget 显式传入 `xxxColor` 非 null → 直接使用
2. `tokens.xxxColor` 非 null → 使用 token 值
3. 两者均 null → 抛 `IllegalStateException`（SDK 配置错误，不静默 fallback）

### widget API 中的 color 参数

所有容器 / 文本 / 按钮 widget 都增加了对应的 color 参数：

```kotlin
Container(fillColor = PixelColor.fromRgb(255, 0, 0))
Text("hello", color = PixelColor.fromRgb(0, 0, 255))
OutlinedButton("OK", onPressed = null, borderColor = PixelColor.fromRgb(0, 255, 0))
```

在 Mono 模式下传入 color 参数会在 debug 构建中抛出 `IllegalStateException`，release 静默忽略。

### PixelColor API

```kotlin
// 构造
PixelColor.fromRgb(r, g, b)           // alpha = 0xFF
PixelColor.fromArgb(a, r, g, b)
PixelColor.Transparent                // argb = 0

// 读取
color.alpha; color.red; color.green; color.blue
color.argb   // 原始 ARGB Int
```

### 颜色补间动画

```kotlin
val colorTween = PixelColorTween(
    begin = PixelColor.fromRgb(255, 0, 0),
    end = PixelColor.fromRgb(0, 0, 255),
)
// 在 AnimationController 里使用
val current = colorTween.evaluate(animationController)
```

## 接下来

- 不同 widget 怎么用样式参数 → [组件目录](组件目录.md)
- 内置 `PixelPalette` 都有哪些预设 → 看 `PixelPalette` 类的 companion object
- 自定义字体（点阵样式） → [扩展开发](扩展开发.md)
