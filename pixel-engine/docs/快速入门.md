...
> ⚠️ **关于 lazy 渲染**：`ListViewBuilder` **只有传 `itemExtent`（每 item 固定高度）时才走 lazy 路径**，仅构造可见区 + cacheExtent 范围内的 item。如果不传 `itemExtent`，会一次性构造全部 itemCount 个 widget，长列表（> 几十条）会卡顿。详见 [组件目录](组件目录.md#listviewbuilder)。
