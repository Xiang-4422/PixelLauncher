...
- [x] 不演变成自由 widget 大屏。
- [x] 状态块服务 Home 的重要信息判断。
- [x] 每个状态块都能关闭或降级。

证据：

- `HomeStateBlockModel` 只定义官方 `CALENDAR / MEDIA / TODO / COUNTDOWN` 四类状态块，不提供自由 widget 注册入口。
- `HomeStateBlockModelTest` 覆盖默认全关闭、每类状态块、同屏最多一个主状态块、主块优先级、`SUMMARY` 降级和 `OFF` 关闭。
- 当前只落地选择模型和验收规则，不引入外部日历、媒体或 Todo 数据源；后续接入数据时必须继续通过该模型裁决。
...
- Home 重要信息优先级由 `HomeInfoModel` 和 `HomeStateBlockModel` 共同约束，通信和数据异常优先，官方状态块最多显示一个主块。
- Notification Listener 配置采用 App 级 `NORMAL / PRIORITY / MUTED` 三态，不进入完整通知中心。
- SMS 联系人映射优先读取系统联系人；无联系人权限或无匹配时回退到号码归一化展示。
- `pixel-demo` 不是本任务列表的阻塞项；Launcher UI 规范先由 App 自身页面和静态测试验收。
