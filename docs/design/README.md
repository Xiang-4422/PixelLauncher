...
- [像素 UI 引擎执行计划与并行开发指南](./engine/像素UI引擎执行计划与并行开发指南.md)
...
- 当前真正的执行口径已经收敛到引擎文档、当前进度文档和执行计划文档，不再维护多套临时并行任务表
...
5. 读 [像素 UI 引擎执行计划与并行开发指南](./engine/像素UI引擎执行计划与并行开发指南.md)
6. 如果要直接基于引擎开发，读 [像素 UI 引擎组件接入指南](./engine/像素UI引擎组件接入指南.md)
7. 如果涉及渲染、性能或 Idle 动画，读 [渲染实现原理](./architecture/渲染实现原理.md)
...
3. 读 [像素 UI 引擎执行计划与并行开发指南](./engine/像素UI引擎执行计划与并行开发指南.md)
4. 读 [像素 UI 引擎组件接入指南](./engine/像素UI引擎组件接入指南.md)
5. 再看 `pixel-demo` 和对应 runtime 代码

## 4. 文档维护规则

- 技术真实实现变化，先更新 `architecture/` 和 `engine/`
- 产品目标或模块职责变化，更新 `product/`
- 不再新增临时任务单或一次性完成度清单；当前阶段固定维护 [像素 UI 引擎执行计划与并行开发指南](./engine/像素UI引擎执行计划与并行开发指南.md) 作为正式执行口径
