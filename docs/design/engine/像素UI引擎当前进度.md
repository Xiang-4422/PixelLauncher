...
- `CustomScrollView` + eager/lazy `SliverList` + pinned header + collapsible `SliverAppBar`
...
- `CustomScrollView` 已有 eager `SliverList`、固定 extent lazy `SliverListBuilder`、pinned header 和 collapsible `SliverAppBar` v1，但还没有变高 lazy sliver、多个 pinned header/app bar 的推挤规则或 floating/snap/stretch app bar
