...
import com.purride.pixellauncherv2.launcher.FontFaceDescriptor
...
/** Launcher 字体异步准备过程的公开状态。 */
sealed interface LauncherFontLoadState {
    /** 尚未发起冷启动字体请求。 */
    data object Idle : LauncherFontLoadState

    /** 旧字体保持可用、候选字体正在后台准备。 */
    data class Loading(
        /** 当前已激活字体；冷启动时为 null。 */
        val active: LauncherFontSelection?,
        /** 正在准备的候选字体。 */
        val pending: LauncherFontSelection,
    ) : LauncherFontLoadState
    ...
    data class Ready(
        ...
        val active: LauncherFontSelection,
        /** 当前完整 typography。 */
        val prepared: PreparedLauncherFont,
    ) : LauncherFontLoadState
    ...
    data class Failed(
        ...
        val active: LauncherFontSelection?,
        /** 加载失败的候选字体。 */
        val requested: LauncherFontSelection,
        /** 原始加载异常。 */
        val error: Throwable,
    ) : LauncherFontLoadState
}
...
internal data class PreparedFontFace(
    /** catalog 精确 face。 */
    val descriptor: FontFaceDescriptor,
...
/** 已完整准备、渲染时不再执行资源 IO 的 Launcher 字体。 */
class PreparedLauncherFont internal constructor(
    /** 设置页当前激活的原生 face。 */
    val selection: LauncherFontSelection,
    /** 原生和组件 face 的精确索引。 */
    private val faces: Map<LauncherFontSelection, PreparedFontFace>,
) {
    /** 向组件暴露的精确字号 typography。 */
    val typography: LauncherTypography = LauncherTypography(selection, ::rasterizer)
    ...
    fun rasterizer(faceSelection: LauncherFontSelection): PixelTextRasterizer =
        requireNotNull(faces[faceSelection]) { "Font face was not prepared: $faceSelection" }.rasterizer

    /** 返回首字形真实墨迹前距；前导空格作为内容保留。 */
    fun leadingInkInset(text: String, faceSelection: LauncherFontSelection): Int {
    ...
    fun trailingInkInset(text: String, faceSelection: LauncherFontSelection): Int {
        if (text.isEmpty()) return 0
        /** 文本末尾的完整 Unicode 标量。 */
    ...
    fun measureTextWidth(text: String, faceSelection: LauncherFontSelection): Int =
...
/** 异步加载、single-flight、有限历史和原子准备 Launcher 字体。 */
class LauncherFontRepository(
    ...
    private val preparedCache = LinkedHashMap<LauncherFontSelection, PreparedLauncherFont>(4, 0.75f, true)
    /** 当前调用方持有的可取消等待句柄。 */
    ...
    fun prepare(
        ...
        onComplete: (Result<PreparedLauncherFont>) -> Unit,
    ) {
        check(!disposed) { "LauncherFontRepository is disposed" }
        require(PixelFontCatalog.supports(selection)) { "Font selection is not settings-visible: $selection" }
        ...
        val handlesBySelection = requiredSelections.associateWith { faceSelection ->
            PixelFontCatalog.requireFace(faceSelection).packs.map { pack ->
        ...
        executor.execute {
            val result = runCatching {
                val preparedFaces = handlesBySelection.mapValues { (faceSelection, handles) ->
                    val descriptor = PixelFontCatalog.requireFace(faceSelection)
                ...
                PreparedLauncherFont(selection, preparedFaces)
            }
            mainHandler.post {
    ...
    fun trimToActive(active: LauncherFontSelection) {
    ...
    fun clearGlyphCaches() {
        synchronized(preparedCache) { preparedCache.values.forEach(PreparedLauncherFont::clearGlyphCaches) }
    }
    ...
    fun dispose() {
        ...
        cancelPendingHandles()
        synchronized(preparedCache) {
            preparedCache.values.forEach(PreparedLauncherFont::clearGlyphCaches)
            preparedCache.clear()
        }
        resourceCache.clear()
    }
    ...
    private fun requiredSelections(selection: LauncherFontSelection): List<LauncherFontSelection> = listOf(
        selection,
    ...
    private fun validatePacks(descriptor: FontFaceDescriptor, packs: List<PixelIndexedGlyphPack>) {
    ...
    private fun styleFor(metrics: PixelFontMetrics): GlyphStyle = GlyphStyle(
    ...
    private fun cachePrepared(prepared: PreparedLauncherFont) {
        synchronized(preparedCache) {
