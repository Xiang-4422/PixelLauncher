...
/**
 * Coverage for [DiagnosticsModel.lines] — the six diagnostic rows (titles +
...
class DiagnosticsModelTest {
    ...
    @Test
    fun lines_buildsSixRowsInExpectedOrder() {
        val titles = DiagnosticsModel.lines(LauncherState(), profile).map { it.title }
        assertEquals(listOf("LAUNCHES", "LAST", "RECENT", "FONT", "DISPLAY", "POWER"), titles)
    }
