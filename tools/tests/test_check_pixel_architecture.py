...
class PixelArchitectureGovernanceTest(unittest.TestCase):
    ...
    def write_architecture_contract(self, root: Path) -> None:
        ...
        (root / "settings.gradle.kts").write_text(
            'include(":app", ":pixel-engine", ":showcase", ":showcase-desktop", ":lockscreen-module", ":pixel-design")\n',
            encoding="utf-8",
        )
        # 关键文档共享同一模块清单受控区段。
        module_block = "\n".join(
    ...
    def test_accepts_current_six_module_contract(self) -> None:
        """Gradle 清单及关键文档一致声明六模块时必须通过。"""
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            budget = self.write_budget(root)
            (root / "settings.gradle.kts").write_text(
                'include(":pixel-design", ":lockscreen-module", ":showcase-desktop", ":showcase", ":pixel-engine", ":app")\n',
                encoding="utf-8",
            )
    ...
    def test_gradle_module_parser_ignores_line_and_block_comments(self) -> None:
        ...
        settings_text = """
            ...
            */
            include(":pixel-engine", ":showcase", ":showcase-desktop", ":lockscreen-module", ":pixel-design")
        """
    ...
    def test_rejects_commented_out_gradle_module(self) -> None:
        """settings 仅在注释中保留视觉模块时必须报告清单漂移。"""

        with tempfile.TemporaryDirectory() as directory:
            # 完整文档保持不变，视觉模块的 include 被行注释移除。
            root = Path(directory)
            (root / "pixel-engine/src/main/kotlin").mkdir(parents=True)
            budget = self.write_budget(root)
            (root / "settings.gradle.kts").write_text(
                'include(":app", ":pixel-engine", ":showcase")\n'
                'include(":showcase-desktop", ":lockscreen-module")\n'
            ...
            self.assertEqual(":lockscreen-module", finding["actual"][-1])
            self.assertEqual(":pixel-design", finding["expected"][-1])
    ...
    def test_rejects_key_document_module_list_drift(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            overview.write_text(
                overview.read_text(encoding="utf-8").replace("- `:pixel-design`\n", ""),
                encoding="utf-8",
            )
            ...
            self.assertNotIn(":pixel-design", finding["actual"])
